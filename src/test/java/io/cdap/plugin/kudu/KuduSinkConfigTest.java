...
public class KuduSinkConfigTest {
  private static final String MOCK_STAGE = "mockStage";
  private static final KuduSinkConfig VALID_CONFIG = new KuduSinkConfig(
    "kuduSink",
    "kuduTable",
  ...
  public void testValidateIncorrectReferenceName() {
    MockFailureCollector failureCollector = new MockFailureCollector(MOCK_STAGE);
    KuduSinkConfig config = KuduSinkConfig.builder(VALID_CONFIG)
      .setReferenceName("!!!")
      .build();
    List<List<String>> paramNames = Collections.singletonList(
      Collections.singletonList(KuduSinkConfig.REFERENCE_NAME)
    );
  ...
  public void testValidateIncorrectSchema() {
    ...
    KuduSinkConfig config = KuduSinkConfig.builder(VALID_CONFIG)
      ...
      .build();
    List<List<String>> paramNames = Collections.singletonList(
      Collections.singletonList(KuduSinkConfig.SCHEMA));
  ...
  public void testValidateIncorrectMasterAddresses() {
    ...
    List<List<String>> paramNames = Arrays.asList(
      Collections.singletonList(KuduSinkConfig.MASTER),
      Collections.singletonList(KuduSinkConfig.MASTER)
    );
